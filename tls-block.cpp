...
static void inject_forward_rst(pcap_t* handle,
    ...
    tcph->rst = 1; tcph->ack = 1;
    tcph->th_off = tcp_sz/4;
    tcph->check = 0;

    struct { uint32_t src,dst; uint8_t zero,proto; uint16_t len; } pseudo;
    pseudo.src = iph->saddr; pseudo.dst = iph->daddr;
    pseudo.zero = 0; pseudo.proto = IPPROTO_TCP; pseudo.len = htons(tcp_sz);

    memcpy(outbuf, orig, eth_sz); // restore eth header after pseudo calc? actually separate
...
static void inject_backward_rst(const iphdr* iph_orig,
    ...
    iph->saddr = iph_orig->daddr; iph->daddr = iph_orig->saddr;
    iph->tot_len = htons(pkt_sz); iph->check = 0;
    iph->check = checksum((u16*)iph, ip_sz);

    auto* tcph = (tcphdr*)(pkt.data()+ip_sz);
    tcph->source = tcph_orig->dest; tcph->dest = tcph_orig->source;
    tcph->seq = tcph_orig->ack_seq;
    tcph->ack_seq = htonl(ntohl(tcph_orig->seq)+data_len);
    tcph->rst = 1; tcph->ack = 1;
    tcph->th_off = tcp_sz/4; tcph->check = 0;
...
int main(int argc, char** argv) {
    ...
    if(!handle){ cerr<<ebuf<<"\n"; return 1;} pcap_set_immediate_mode(handle,1);
    struct bpf_program fp; pcap_compile(handle,&fp,"tcp dst port 443",1,PCAP_NETMASK_UNKNOWN);
    pcap_setfilter(handle,&fp);

    // get MAC
    uint8_t mac[6]; int fd=socket(AF_INET,SOCK_DGRAM,0);
    struct ifreq ifr{}; strncpy(ifr.ifr_name,dev,IFNAMSIZ-1);
    ioctl(fd,SIOCGIFHWADDR,&ifr); memcpy(mac,ifr.ifr_hwaddr.sa_data,6);
    close(fd);
    ...
    while(true){
        ...
        if(!r.in_progress){
            ...
            if(dlen>=5 && data[0]==0x16){
                uint16_t rec=(data[3]<<8)|data[4]; r.base_seq=seq;
                r.expected_len=5+rec; r.buf.clear(); r.buf.reserve(r.expected_len);
                r.buf.insert(r.buf.end(),data,data+dlen); r.in_progress=true;
            }
        } else {
            if(seq>=r.base_seq && seq<r.base_seq+r.expected_len){
                ...
                if(r.buf.size()>=r.expected_len){
                    string sni=parse_sni(r.buf.data(),r.buf.size()); flows.erase(key);
                    if(!sni.empty() && sni.find(pat)!=string::npos){
