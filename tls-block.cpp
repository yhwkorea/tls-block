...
unsigned short checksum(u16* buf, int len) {
    unsigned long sum = 0;
...
int main(int argc, char* argv[]) {
    ...
    if(!h){cerr<<err;return-1;} pcap_set_immediate_mode(h,1);
    map<Flow,Reassembly> flows;

    while(true){
        pcap_pkthdr* hdr; const u_char* pkt;
        if(pcap_next_ex(h,&hdr,&pkt)<=0) continue;
        ip* iph=(ip*)(pkt+14); if(iph->ip_p!=IPPROTO_TCP) continue;
        int iph_l=iph->ip_hl*4; tcphdr* tcph=(tcphdr*)(pkt+14+iph_l);
        int tcph_l=tcph->th_off*4;
        int dlen=ntohs(iph->ip_len)-iph_l-tcph_l;
        if(dlen<=0) continue;
        const uint8_t* data=pkt+14+iph_l+tcph_l;

        Flow f{iph->ip_src,iph->ip_dst,ntohs(tcph->th_sport),ntohs(tcph->th_dport)};
        auto &r=flows[f]; uint32_t seq=ntohl(tcph->th_seq);

        if(r.buf.empty()){
            string sni=parse_sni(data,dlen);
            if(!sni.empty() && sni.find(pat)!=string::npos){
                send_rst(iph,tcph,dlen);
                ip ir=*iph; tcphdr tr=*tcph;
                ir.ip_src=iph->ip_dst; ir.ip_dst=iph->ip_src;
                tr.th_sport=tcph->th_dport; tr.th_dport=tcph->th_sport;
                tr.th_seq=tcph->th_ack;
                tr.th_ack=htonl(ntohl(tcph->th_seq)+dlen);
                send_rst(&ir,&tr,dlen);
                cout<<"[+] TLS-blocked "<<sni<<"\n";
            } else if(dlen>=5 && data[0]==0x16){
                uint16_t rec=(data[3]<<8)|data[4];
                r.base_seq=seq; r.expected=5+rec; r.got_len=true;
            ...
            if(seq==r.base_seq+r.buf.size()){
                ...
                if(!r.got_len && r.buf.size()>=5){
                    uint16_t rec=(r.buf[3]<<8)|r.buf[4]; r.expected=5+rec; r.got_len=true;
                }
                if(r.got_len && r.buf.size()>=r.expected){
                    string sni=parse_sni(r.buf.data(),r.buf.size()); flows.erase(f);
